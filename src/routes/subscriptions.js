...
module.exports = async function(fastify, opts) {
    ...
    fastify.get('/create', { preHandler: verifySubscriptionAccess }, async (request, reply) => {
        try {
            ...
            const customers = await db.query(`
                SELECT id, name as nama, phone as nomor_hp, is_active as status_aktif
                FROM customers
                ORDER BY name
            `);
    ...
    // Create Subscription
    fastify.post('/api/subscriptions', ApiErrorHandler.asyncHandler(async (request, reply) => {
        ...
        if (!customer_id || !service_type || !profile_id || !username || !price_sell) {
            return ApiErrorHandler.validationError(reply, 'Field wajib tidak boleh kosong');
        }
        ...
        if (existingSubscription) {
            return ApiErrorHandler.validationError(reply, 'Username sudah digunakan');
        }
    ...
    // Get Subscription Details
    fastify.get('/api/subscriptions/:id', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { id } = request.params;
        ...
        if (!subscription) {
            return ApiErrorHandler.notFoundError(reply, 'Berlangganan tidak ditemukan');
        }
    ...
    // Update Subscription
    fastify.put('/api/subscriptions/:id', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { id } = request.params;
        ...
        const currentSubscription = await db.getOne('SELECT * FROM subscriptions WHERE id = $1', [id]);
        ...
        if (username !== currentSubscription.username) {
            const existingSubscription = await db.getOne(
                'SELECT id FROM subscriptions WHERE username = $2 AND id != $3',
                [username, id]
            );
        ...
        await db.query(`
            ...
            SET customer_id = $4, service_type = $5, profile_id = $6, username = $7,
                price_sell = $8, price_cost = $9, billing_cycle = $10, duration_days = $11,
                expiry_date = $12, status = $13, auto_renew = $14, updated_at = CURRENT_TIMESTAMP
                WHERE id = $15
        `, [
    ...
    // Delete Subscription
    fastify.delete('/api/subscriptions/:id', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { id } = request.params;

        // Get subscription details before deletion
        const subscription = await db.getOne('SELECT * FROM subscriptions WHERE id = $1', [id]);
        if (!subscription) {
            return ApiErrorHandler.notFoundError(reply, 'Berlangganan tidak ditemukan');
        }

        // Delete subscription
        await db.query('DELETE FROM subscriptions WHERE id = $2', [id]);
    ...
    fastify.get('/api/subscriptions', ApiErrorHandler.asyncHandler(async (request, reply) => {
    ...
    fastify.get('/api/subscriptions/statistics', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const stats = {
            total: 0,
            active: 0,
            disabled: 0,
    ...
    fastify.get('/api/subscriptions/export', ApiErrorHandler.asyncHandler(async (request, reply) => {
    ...
    // Renew Subscription
    fastify.post('/api/subscriptions/:id/renew', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { id } = request.params;
        ...
        await db.query(`
            UPDATE subscriptions
            SET expiry_date = $2, status = \'active\', updated_at = CURRENT_TIMESTAMP
            WHERE id = $3
        ...
        if (amount && payment_method) {
            await db.query(`
                ...
                ) VALUES ($4, $5, $6, $7, 'paid', $8, CURRENT_TIMESTAMP)
            `, [
    ...
    // Bulk Operations
    fastify.post('/api/subscriptions/bulk-renew', ApiErrorHandler.asyncHandler(async (request, reply) => {
        ...
        if (!subscription_ids || !Array.isArray(subscription_ids) || subscription_ids.length === 0) {
            return ApiErrorHandler.validationError(reply, 'Tidak ada berlangganan yang dipilih');
        }
        ...
        for (const subscriptionId of subscription_ids) {
            try {
                const subscription = await db.getOne('SELECT * FROM subscriptions WHERE id = $1', [subscriptionId]);
                if (!subscription) {
    ...
    // Bulk Disable
    fastify.post('/api/subscriptions/bulk-disable', ApiErrorHandler.asyncHandler(async (request, reply) => {
    ...
    // Bulk Delete
    fastify.post('/api/subscriptions/bulk-delete', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { subscription_ids } = request.body;
    ...
    // Bulk Send Reminders
    fastify.post('/api/subscriptions/bulk-reminder', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { subscription_ids } = request.body;
    ...
    // Sync with Mikrotik
    fastify.post('/api/subscriptions/sync-mikrotik', ApiErrorHandler.asyncHandler(async (request, reply) => {
        if (!fastify.mikrotik) {
    ...
    fastify.post('/api/subscriptions/:id/sync-mikrotik', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { id } = request.params;
    ...
    fastify.post('/api/subscriptions/:id/disable', ApiErrorHandler.asyncHandler(async (request, reply) => {
        const { id } = request.params;
